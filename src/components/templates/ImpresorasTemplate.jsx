...
import {HeaderImpresoras} from "../organismos/ImpresoraDesing/HeaderImpresoras";
...
export const ImpresorasTemplate = () => {
  ...
  const { data: dataSucursalYcaja, error: errorDataSucursalYCaja} = useQuery({
    queryKey: ["mostrar caja, sucursal"],
    queryFn: () => mostrarSucursalCajaAsignada({
      id_usuario: dataSucursalesAsignadas[0]?.id_usuario})
    })
  ...
  const {
    ...
    queryKey: [
      "mostrar impresora por caja",
      {
        ...
        //id_caja: 500
        id_caja: dataSucursalYcaja?.id_caja
      },
    ],
    queryFn: () =>
      mostrarImpresoraXCaja({
