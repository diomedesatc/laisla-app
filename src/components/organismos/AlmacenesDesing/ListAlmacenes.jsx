...
export function ListAlmacenes(){
    ...
    const {mostrarCajasXSucursal, selectSucursal, setAccion, setStateSucursal, eliminarSucursal} = useSucursalesStore();
    const {dataEmpresa} = useEmpresaStore();
    const {setStateCaja, setCajaSelectItem, setAccion: setAccionCaja, eliminarCaja} = useCajaStore();
    const {mostrarAlmacenesXEmpresa, setStateAlmacen, setAlmacenItemSelect, setAccion: setAccionAlmacen, eliminarAlmacen} = useAlmacenesStore();

    const {data, isLoading, error} = useQuery({
        queryKey: ["mostrar almacenes por empresa"],
                        ...
                        <Sucursal key={index}>
                            <SucursalHeader>
                                <Acciones $right="0px" $top="0px">
                                    {
                                        sucursal.delete && 
                                        <Icon icon="wpf:delete" width="15" height="15" className="deleteicon" onClick={() => doDeleteSucursal(sucursal?.id)}/>
                                    }
                                    <Icon icon="mdi:edit" width="20" height="20" onClick={() => editarSucursal(sucursal)}/>
                                </Acciones>
                                        ...
                                        return(
                                            <CajaItem key={index}>
                                                ...
                                                <Acciones $right="10px" $bottom="10px">
                                                    {
                                                        almacen?.delete && 
                                                    <Icon icon="wpf:delete" width="15" height="15" className="deleteicon" onClick={() => doDeleteAlmacen(almacen?.id)}/>
                                                    }
                                                    <Icon icon="mdi:edit" width="20" height="20" onClick={() => editarAlmacen(almacen)}/>
                                                </Acciones>
                            ...
                            <ButtonDashed title="Agregar almacen" funcion={() => agregarAlmacen(sucursal)} />
                        </Sucursal>
